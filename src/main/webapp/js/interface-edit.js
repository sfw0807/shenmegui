$(function () {
    ...
    var initInputs = function initInputs(result) {
        ...
        $('#serviceIds').append("<option value='" + result.serviceId + "'>" + result.service_ID + "</option>");
    ...
    var initOperations = function (result) {
        $("#operationIds").find("option").remove();
        for (var i = 0; i < result.length; i++)
            $('#operationIds').append("<option value='" + result[i].operationId + "'>" + result[i].operationId + "</option>");
    };


    $('#serviceIds').change(function () {
        var serviceId = $('#serviceIds option:selected').text();
        interfaceManager.getOperationOfService(serviceId, initOperations);
    });
    ...
    $('#save').button().click(function () {
        var params = {
            serviceId: $('#serviceIds option:selected').text(),
    ...
    $("#editIDA").click(function () {
        if (isChrome) {
            ...
            return window.open("../jsp/ida.jsp?interfaceId=" + ecode, window, winOption);
        } else {
            window.showModalDialog("../jsp/ida.jsp?interfaceId=" + ecode, "", "dialogWidth:1200px;dialogHeight:800px;resizable=yes");
        }
